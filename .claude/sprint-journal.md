# Sprint Journal Index — PrivateCloud

> One file per task. Newest at top. Read any entry to prep for teacher Q&A.

**Folder:** `.claude/journal/`
**Naming:** `NNN-short-title.md` (zero-padded, sequential)

## Entries

| # | Date | Title | Sprint | File |
|---|------|-------|--------|------|
| 020 | 2026-05-27 | Clone-from-Template: Teacher Templates → Bulk-Clone to a Class | 5 | [020-clone-from-template.md](journal/020-clone-from-template.md) |
| 019 | 2026-05-26 | RAG Knowledge Base Integration into the ChatOps Agent | 4 | [019-rag-knowledge-base-integration.md](journal/019-rag-knowledge-base-integration.md) |
| 018 | 2026-05-25 | Recover vm_ip When Initial Provisioning Failed | 4 | [018-vm-ip-recovery-after-failed-provisioning.md](journal/018-vm-ip-recovery-after-failed-provisioning.md) |
| 017 | 2026-05-24 | Cross-Device Desktop Access (Guacamole URL Rewrite) | 4 | [017-cross-device-desktop-access.md](journal/017-cross-device-desktop-access.md) |
| 016 | 2026-05-24 | Fix Celery Auto-Discovery and Worker Healthcheck | 3 | [016-celery-worker-fix.md](journal/016-celery-worker-fix.md) |
| 015 | 2026-04-27 | Iteration 3 UML Diagrams (Class + Sequence, PlantUML) | 3 | [015-i3-uml-diagrams.md](journal/015-i3-uml-diagrams.md) |
| 014 | 2026-04-20 | Admin Portal DB Schema Extensions (I3: 4 additions) | 3 | [014-admin-db-schema-extensions.md](journal/014-admin-db-schema-extensions.md) |
| 013 | 2026-04-19 | VM Detail Page: Provisioning State UX (fix 409 on Start) | 3 | [013-vm-provisioning-ux.md](journal/013-vm-provisioning-ux.md) |
| 012 | 2026-04-15 | Code Review: Security & Quality Fixes (2C, 12H, 15M fixed) | 3 | [012-code-review-security-fixes.md](journal/012-code-review-security-fixes.md) |
| 011 | 2026-04-15 | Admin Portal: Full-Stack Implementation | 3 | [011-admin-portal-fullstack.md](journal/011-admin-portal-fullstack.md) |
| 010 | 2026-04-14 | Fix VM Node Name Bug & Console Access Research | 2 | [010-fix-vm-node-name-and-console-research.md](journal/010-fix-vm-node-name-and-console-research.md) |
| 009 | 2026-04-13 | Fix Login Case-Sensitivity Bug | 2 | [009-login-case-sensitivity-bug.md](journal/009-login-case-sensitivity-bug.md) |
| 008 | 2026-04-12 | Fix All 8 Code Review Issues from Golden Image Integration | 2 | [008-code-review-fixes.md](journal/008-code-review-fixes.md) |
| 007 | 2026-04-12 | VM Creation: ISO Boot Order Fix & use_template Flag | 2 | [007-vm-creation-iso-boot-fix.md](journal/007-vm-creation-iso-boot-fix.md) |
| 006 | 2026-04-12 | Proxmox VM Rebuild & Environment Reconfiguration | 2 | [006-proxmox-rebuild-env-update.md](journal/006-proxmox-rebuild-env-update.md) |
| 005 | 2026-04-04 | Sprint 2 Code Review, azams-branch Push, Kanban Update | 2 | [005-sprint2-code-review-branch-push.md](journal/005-sprint2-code-review-branch-push.md) |
| 004 | 2026-04-03 | Enhanced VM List with Live Proxmox Status | 2 | [004-enhance-vm-list.md](journal/004-enhance-vm-list.md) |
| 003 | 2026-04-03 | Update VM Endpoint (Start/Stop/Restart/Resize) | 2 | [003-update-vm-endpoint.md](journal/003-update-vm-endpoint.md) |
| 002 | 2026-04-03 | Token-Based Proxmox API Auth | 2 | [002-token-based-proxmox-auth.md](journal/002-token-based-proxmox-auth.md) |
| 001 | 2026-04-02 | Sprint 1 Retrospective — Full Backend | 1 | [001-sprint1-backend-implementation.md](journal/001-sprint1-backend-implementation.md) |
