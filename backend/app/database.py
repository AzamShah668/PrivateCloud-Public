...
# database.py
# =============================================================================
# Your colleague's database layer — the ONLY file that talks to PostgreSQL.
#
# Architecture choice: raw psycopg2 + ThreadedConnectionPool
# ────────────────────────────────────────────────────────────────────────────
# Unlike SQLAlchemy (which maps Python classes to tables), this file uses
# raw SQL strings. That means:
#   ✓ No ORM magic — every query is explicit and easy to read
#   ✓ Full control over the exact SQL being sent to Postgres
#   ✓ Lighter weight — no ORM overhead at all
#   ✗ No automatic migrations — schema is managed here via CREATE TABLE IF NOT EXISTS
#
# Connection pooling with ThreadedConnectionPool
# ────────────────────────────────────────────────────────────────────────────
# Opening a new PostgreSQL connection for every request is expensive (~50ms).
# A pool keeps a set of connections open permanently. Each request BORROWS
# one, uses it, then RETURNS it. This is the _conn() context manager below.
#
# RealDictCursor
# ────────────────────────────────────────────────────────────────────────────
# By default psycopg2 returns rows as tuples: row[0], row[1], ...
# RealDictCursor makes rows behave like dicts: row["id"], row["username"], ...
# Much more readable and less fragile when columns are added/reordered.
#
# Public functions exposed by this module (called from routes):
#   init_db()                    → call once at startup
#   create_user(...)             → insert a user, return id
#   get_user_by_username(...)    → return user dict or None
#   get_user_by_id(...)          → return user dict or None
#   count_user_jobs_today(...)   → return int (for quota check)
#   create_vm_job(...)           → insert a vm_job, return id
#   update_vm_job(...)           → update status / response / error
#   get_vm_job(...)              → return vm_job dict or None
#   list_user_vm_jobs(...)       → return list of vm_job dicts
#   add_audit_log(...)           → insert an audit_logs row
#   list_audit_logs(...)         → return list of audit dicts
# =============================================================================
...
def _conn():
    ...
    If an exception is raised inside the `with` block, the connection is
    automatically rolled back before being returned to the pool — this
    prevents a half-written transaction from being used by the next request.
    """
    pool = _get_pool()
    conn = pool.getconn()
    try:
...
def init_db() -> None:
    ...
    Called from main.py in the FastAPI lifespan event:
        @asynccontextmanager
